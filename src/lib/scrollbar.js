  ...
  function(module, exports, __webpack_require__) {
    ...
    exports.default = {
      ...
      computed: {
        ...
        _left() {
          var left = Math.max(0, Math.min(this.left, this.scrollContentWidth - this.scrollContainerWidth))
          if (this.headerEl && this.scrollContentWidth === this.headerContentEl.clientWidth) {
      ...
      methods: {
        ...
        scroll: function scroll(e) {
          //e.preventDefault();
          if (this.$store.state.menuStatus && (this.scrollContainerHeight === folder_box.clientHeight || this.$el === this.$store.getters.thumbDisplay.el))
            return;
      ...
      mounted: function mounted() {
        ...
        this.headerContentEl = document.querySelector('.list_header_container')
  ...
  function(module, exports) {
    ...
    exports.default = {
      ...
      watch: {
        ...
        'resizeID': {
          handler: function(val, old) {
            if (val !== old && this.$parent.$el === this.$store.getters.thumbDisplay.el) { //改为配置
              this.parentCalc()
        ...
        'scrolling.v': {
          handler: util.throttle(30, function(val, old) {
            if (val != old && this.$parent.$el === this.$store.getters.thumbDisplay.el) { //改为配置
        ...
        currentNode: {
          handler: function(val, old) {
            if (val != old && this.$parent.$el === this.$store.getters.thumbDisplay.el) {
              this.onChangePosition(0, 'vertical');
            }
            if (val != old && this.container.height == folder_box.clientHeight) {
              Vue.nextTick(() => {
                this.parentCalc();
        ...
        selectedMaterials: {
          handler: function(val, old) {
            if (val.length && this.$parent.$el === this.$store.getters.thumbDisplay.el) {
              //Vue.nextTick(() => {
              this.parentCalc();
              var item,
                itemHeight,
              ...
              if (this.$store.state.dragSymbol) { //框��?
                if (!this.$store.state.dragDirection) { // up
                  item = val[0]
                  if (!item.guid) {
                    return
                  }
                  itemHeight = item.type === 'marker' ? 102 : this.$store.getters.itemHeight;
                  top = $('[data-id="' + item.guid + '"]').position().top
                  offsetTop = $('[data-id="' + item.guid + '"]').offset().top;
                } else { //down
                  item = val[val.length - 1]
                  itemHeight = item.type === 'marker' ? 102 : (14 + this.$store.state.thumbnailStyle.height * this.$store.state.scaleTime + 45);
                  top = $('[data-id="' + item.guid + '"]').position().top
                  offsetTop = $('[data-id="' + item.guid + '"]').offset().top;
                }
                if (top === 0) {
                  ...
                  this.onChangePosition(Math.max(0, (top - itemHeight) / this.content.height * 100), 'vertical');
                } else if (offsetTop > this.container.height - 83) {
                  this.onChangePosition((top - (this.container.height - 93 - itemHeight)) / this.content.height * 100, 'vertical');
                }
              } else { //快捷键选择
          ...
          Vue.nextTick(() => {
            ...
            if (this.container.height == folder_box.clientHeight) {
              ...
              if (offsetTop > this.container.height) {
                this.onChangePosition((top - (this.container.height - 80)) / this.content.height * 100, 'vertical');
      ...
      methods: {
        scrollTo(item, isMarker) {
          ...
          var rowCount = isMarker ? Math.floor(width / itemWidth) : Math.round((width - 2 * this.$store.state.thumbPadding) / itemWidth)
          if (index <= info.index || index >= info.index + info.range) {
            this.onChangePosition(Math.floor(index / info.rowCount - 1) * info.cellPercent, 'vertical');
          } else {
            top = $('[data-id="' + item.guid + '"]').position().top
            offsetTop = $('[data-id="' + item.guid + '"]').offset().top;
            if (top === 0) {
              ...
              this.onChangePosition(Math.max(0, (top - itemHeight) / this.content.height * 100), 'vertical');
            } else if (offsetTop > this.container.height - itemHeight + 52 + condtionHeight) {
              this.onChangePosition((top - (this.container.height - 52 - itemHeight)) / this.content.height * 100, 'vertical');
            }
          }
        },
        calculateSize: function calculateSize() {
